...
const derefByIndex = (index: number, type: string) => {
  if (type === 'char') {
    return String.fromCharCode(RTS.get_word_at_index(RTS.get_word_at_index(index)))
  }
...
export const evaluators: { [nodeType: string]: Evaluator<cs.Node> } = {
  ...
  Dereference_i: function* (instr: any, context: Context) {
    ...
    S.push(derefByIndex(index, instr.pointerType))
  },
