...
function makeSequenceIfNeeded(exs: cs.Node[]) {
  return exs.length === 1
    ? transform(exs[0])
    : vector_to_list(['sequence', vector_to_list(exs.map(transform))])
}

function makeBlockIfNeeded(exs: cs.Node[]) {
...
function hasDeclarationAtToplevel(exs: cs.Node[]) {
  return exs.reduce(
...
type ASTTransformers = Map<string, (node: cs.Node) => Value>

const transformers: ASTTransformers = new Map([
  [
    'Program',
    (node: cs.Node) => {
      node = node as cs.Program
    ...
    (node: cs.IfStatement) => {
      return vector_to_list([
        ...
        node.alternate === null
          ...
          : transform(node.alternate as cs.Statement)
      ])
    }
  ],
  ...
  [
    'FunctionDeclaration',
    (node: cs.FunctionDeclaration) => {
      return vector_to_list([
        'function_declaration',
        transform(node.id as cs.Identifier),
  ...
  [
    'VariableDeclaration',
    (node: cs.VariableDeclaration) => {
      if (node.kind === 'let') {
        return vector_to_list([
          'variable_declaration',
          transform(node.declarations[0].id),
          transform(node.declarations[0].init as cs.Expression)
        ])
  ...
  [
    'ReturnStatement',
    (node: cs.ReturnStatement) => {
      return vector_to_list(['return_statement', transform(node.argument as cs.Expression)])
    }
  ],

  [
    'CallExpression',
    (node: cs.CallExpression) => {
      return vector_to_list([
        'application',
  ...
  [
    ...
    (node: cs.ArrowFunctionExpression) => {
      return vector_to_list([
        'lambda_expression',
        vector_to_list(node.params.map(transform)),
  ...
  [
    ...
    (node: cs.AssignmentExpression) => {
      if (node.left.type === 'Identifier') {
        return vector_to_list([
          'assignment',
          transform(node.left as cs.Identifier),
          transform(node.right)
        ])
      } else if (node.left.type === 'MemberExpression') {
        return vector_to_list([
          'object_assignment',
          transform(node.left as cs.Expression),
          transform(node.right)
        ])
      } else {
        unreachable()
  ...
  [
    'ForStatement',
    (node: cs.ForStatement) => {
      return vector_to_list([
        'for_loop',
        transform(node.init as cs.VariableDeclaration | cs.Expression),
        transform(node.test as cs.Expression),
        transform(node.update as cs.Expression),
        transform(node.body)
      ])
    }
  ],
  ...
  [
    ...
    (_node: cs.ImportDefaultSpecifier) => {
  ...
  [
    'TryStatement',
    (node: cs.TryStatement) => {
      return vector_to_list([
        'try_statement',
        transform(node.block),
        node.handler === null || node.handler === undefined
          ? null
          : vector_to_list(['name', (node.handler.param as cs.Identifier).name]),
...
function transform(node: cs.Node) {
  if (transformers.has(node.type)) {
    const transformer = transformers.get(node.type) as (n: cs.Node) => Value
